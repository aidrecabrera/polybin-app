...
export function fetchDisposeLogFromSupabase(
  page: number,
  pageSize: number,
  startDate?: Date,
  endDate?: Date
  ...
  return useQuery({
    queryKey: ["disposeLog", page, pageSize, startDate, endDate],
    queryFn: async () => {
...
export function fetchDisposeCount() {
  return useQuery({
    queryKey: ["disposeLogCount"],
    queryFn: async () => {
      const { data, error, count } = await supabase
        .from("dispose_log")
        .select("*", { count: "exact" });
